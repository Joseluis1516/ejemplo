...
LEFT JOIN edificios ed ON es.fk_edificios=ed.pk_edificios WHERE u.nom_usuario ='$usuario3' AND  s.estatus=2 GROUP BY pk_solicitud DESC");
...
<body>
    ...
    <div class="datos" align="center">
        <div class="text">
            <p>Presiona si <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-lg" viewBox="0 0 16 16">
                    <path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425a.247.247 0 0 1 .02-.022Z" />
                </svg> te gusto la solucion</p>
            <p>Presiona <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x" viewBox="0 0 16 16">
                    <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z" />
                </svg> si no te gusto la solucion</p>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display: flex; justify-content: space-around;">
                                <button onclick="actualizar_estatus_cero(<?php echo $row['pk_solicitud'] ?>)"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-lg" viewBox="0 0 16 16">
                                        <path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425a.247.247 0 0 1 .02-.022Z" />
                                    </svg></button>
                                <button onclick="actualizar_estatus_uno(<?php echo $row['pk_solicitud'] ?>)"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x" viewBox="0 0 16 16">
                                        <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z" />
                                    </svg></button>
                            </div>
                        </td>
...
<script language="javascript">
    ...
    function actualizar_estatus_uno(pk_solicitud) {
        Swal.fire({
            title: '¿Deseas que se revise la solicitud nuevamente? escribe la razón',
            input: 'textarea',
            ...
            preConfirm: (value) => {
                if (!value) {
                    Swal.showValidationMessage(
                        '<i class="fa fa-info-circle"></i> Escriba la razón'
                    )
                }
            }
        }).then((result) => {
            ...
            $.ajax({
                type: "POST",
                url: "../inserciones/editar_estatus_uno.php",
                data: {
    ...
    function actualizar_estatus_cero(pk_solicitud) {

        Swal.fire({
            title: '¿Se realizo correctamente la solicitud?',
            showCancelButton: true,
            confirmButtonText: 'Si',
            cancelButtonText: 'No',
        }).then((result) => {
            if (result.value) {
                Swal.fire('Sera redirijido a una encuesta');
                $.ajax({
                    type: "POST",
                    url: "../inserciones/editar_estatus_cero.php",
                    data: {
                        'pk_solicitud': pk_solicitud,
                    },
                    dataType: "html",
                    success: function(dato) {
                        location.href = "encuesta.php?pk_solicitud=" + pk_solicitud
                    }
                });
            }
            // location.href = "../inserciones/editar_estatus_cero.php?pk_solicitud=" + pk_solicitud
