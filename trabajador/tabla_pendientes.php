...
INNER JOIN edificios ed ON es.fk_edificios=ed.pk_edificios WHERE s.f_solicitud='$fecha' AND s.estatus=2");
...
<script language="javascript">
    ...
    function actualizar_estatus_uno(pk_solicitud) {
        terminar = confirm('¿Deseas que se revise la solicitud nuevamente?')
        if (terminar) {
            location.href = "../inserciones/editar_estatus_uno.php?pk_solicitud=" + pk_solicitud
        }
    }
    function actualizar_estatus_cero(pk_solicitud) {
        terminar = confirm('¿Se realizo correctamente la solicitud?')
        if (terminar) {
